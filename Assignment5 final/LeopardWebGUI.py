...
class Admin(User):
  ...
  def sys_add_student(self):
    root.title("Admin Add Student Screen")
    ...
    def Logout(event=None):
        ...
        root.bind('<Return>', enter)
        Admin.pick_user_type(self)
    
    def add(event=None):
        stuID = IDTextBox.get().upper()
        stuName = NameTextBox.get().upper()
        stuSurname = SurnameTextBox.get().upper()
        stuGradYR= GradYearTextBox.get().upper()
        stuMajor = MajorTextBox.get().upper()
        stuEmail = EmailTextBox.get().upper()
        stuPassword = PasswordTextBox.get().upper()
        sql_command = f"SELECT ID FROM STUDENT"
        cursor.execute(sql_command)
        stuff = cursor.fetchall()
        for i in range(len(stuff)):
          #  print(stuff[i][0])
           if (stuID == str(stuff[i][0])):
              messagebox.showwarning("Invalid", f"The ID {stuID} Is Already In Use!!")
              break
       
        sql_command = f"SELECT EMAIL FROM STUDENT"
        cursor.execute(sql_command)
        stuff = cursor.fetchall()
        for i in range(len(stuff)):
          #  print(stuff[i][0])
           if (stuEmail == str(stuff[i][0])):
              messagebox.showwarning("Invalid", f"The Email {stuEmail} Is Already In Use!!")
              break
           

        try:
          sql_command = f"INSERT INTO STUDENT VALUES ({stuID}, '{stuName}','{stuSurname}','{stuGradYR}',{stuMajor},'{stuEmail}','{stuPassword}',NULL)"
          cursor.execute(sql_command)
          database.commit()
          messagebox.showwarning("Sucess", f"You Sucessfully Added {stuName} To Students!!")
          

        except:
          errormessage = "Error Please Fill In The Following Entries: \n"
          if(stuID == ""):
          ...
          if(stuGradYR == ""):
             errormessage = errormessage + "Grad Year\n"
          if(stuMajor == ""):
             errormessage = errormessage + "Major\n"
          if(stuEmail == ""):
             errormessage = errormessage + "Email\n"
          if(stuPassword == ""):
             errormessage = errormessage + "Password"

          messagebox.showwarning("Invalid Entry", errormessage)

        


    IDLabel = Label(AdminSearchCourseFrame, text = "ID:", font='8')
    IDLabel.place(relx = 0.45, rely = 0.1, anchor = CENTER, height=50, width=700)

    IDTextBox = tk.Entry(AdminSearchCourseFrame, font='30')
    IDTextBox.place(relx = 0.5, rely = 0.1, anchor = CENTER, height=30, width=500)
    
    NameLabel = Label(AdminSearchCourseFrame, text = "Name:", font='8')
    NameLabel.place(relx = 0.45, rely = 0.2, anchor = CENTER, height=50, width=700)

    NameTextBox = tk.Entry(AdminSearchCourseFrame, font='30')
    NameTextBox.place(relx = 0.5, rely = 0.2, anchor = CENTER, height=30, width=500)
       
    SurnameLabel = Label(AdminSearchCourseFrame, text = "Surname:", font='8')
    SurnameLabel.place(relx = 0.45, rely = 0.3, anchor = CENTER, height=50, width=700)

    SurnameTextBox = tk.Entry(AdminSearchCourseFrame, font='30')
    SurnameTextBox.place(relx = 0.5, rely = 0.3, anchor = CENTER, height=30, width=500)
    
    GradYearLabel = Label(AdminSearchCourseFrame, text = "Grad Year:", font='8')
    GradYearLabel.place(relx = 0.45, rely = 0.4, anchor = CENTER, height=50, width=700)

    GradYearTextBox = tk.Entry(AdminSearchCourseFrame, font='30')
    GradYearTextBox.place(relx = 0.5, rely = 0.4, anchor = CENTER, height=30, width=500)
    
    MajorLabel = Label(AdminSearchCourseFrame, text = "Major:", font='8')
    MajorLabel.place(relx = 0.45, rely = 0.5, anchor = CENTER, height=50, width=700)

    MajorTextBox = tk.Entry(AdminSearchCourseFrame, font='30')
    MajorTextBox.place(relx = 0.5, rely = 0.5, anchor = CENTER, height=30, width=500)
   
    EmailLabel = Label(AdminSearchCourseFrame, text = "Email:", font='8')
    EmailLabel.place(relx = 0.45, rely = 0.6, anchor = CENTER, height=50, width=700)

    EmailTextBox = tk.Entry(AdminSearchCourseFrame, font='30')
    EmailTextBox.place(relx = 0.5, rely = 0.6, anchor = CENTER, height=30, width=500)
   
    PasswordLabel = Label(AdminSearchCourseFrame, text = "Password:", font='8')
    PasswordLabel.place(relx = 0.45, rely = 0.7, anchor = CENTER, height=50, width=700)

    PasswordTextBox = tk.Entry(AdminSearchCourseFrame, font='30')
    PasswordTextBox.place(relx = 0.5, rely = 0.7, anchor = CENTER, height=30, width=500)   
    ...
    LogoutButton = tk.Button(AdminSearchCourseFrame, text = "Return", height=2, width=15, font='30', command=Logout)
    LogoutButton.place(relx = 0.5, rely = 0.9, anchor = CENTER)
  
  def sys_add_Instructor(self):
    root.title("Admin Add Instructor Screen")
    AdminSearchCourseFrame = tk.Frame(root, padx=20,pady=20)
    AdminSearchCourseFrame.pack(fill = "both", expand= True)
    ...
    def add(event=None):
        instID = IDTextBox.get().upper()
        instName = NameTextBox.get().upper()
        instSurname = SurnameTextBox.get().upper()
        instYear= YearTextBox.get().upper()
        instDep = DepartmentTextBox.get().upper()
        instEmail = EmailTextBox.get().upper()
        instPassword = PasswordTextBox.get().upper()

        try:
          sql_command = f"INSERT INTO INSTRUCTOR VALUES ({instID}, '{instName}','{instSurname}',{instYear},'{instDep}','{instEmail}','{instPassword}')"
          cursor.execute(sql_command)
          database.commit()
          messagebox.showwarning("Sucess", f"You Sucessfully Added {instName} To Instructors!!")
        except:
          sql_command = f"SELECT ID FROM INSTRUCTOR"
        
          cursor.execute(sql_command)
          stuff = cursor.fetchall()
          for i in range(len(stuff)):
            print(stuff[i][0])
            if (instID == str(stuff[i][0])):
                messagebox.showwarning("Invalid", f"The ID {instID} Is Already In Use!!")
                break
        
          sql_command = f"SELECT EMAIL FROM INSTRUCTOR"
          cursor.execute(sql_command)
          stuff = cursor.fetchall()
          for i in range(len(stuff)):
            print(stuff[i][0])
            if (instEmail == str(stuff[i][0])):
                messagebox.showwarning("Invalid", f"The Email {instEmail} Is Already In Use!!")
                break
    ...
    YearLabel = Label(AdminSearchCourseFrame, text = "Year:", font='8')
    YearLabel.place(relx = 0.45, rely = 0.4, anchor = CENTER, height=50, width=700)

    YearTextBox = tk.Entry(AdminSearchCourseFrame, font='30')
    YearTextBox.place(relx = 0.5, rely = 0.4, anchor = CENTER, height=30, width=500)
    
    DepartmentLabel = Label(AdminSearchCourseFrame, text = "Department:", font='8')
    DepartmentLabel.place(relx = 0.45, rely = 0.5, anchor = CENTER, height=50, width=700)

    DepartmentTextBox = tk.Entry(AdminSearchCourseFrame, font='30')
    DepartmentTextBox.place(relx = 0.5, rely = 0.5, anchor = CENTER, height=30, width=500)
  ...
  def sys_add_Admin(self):
    root.title("Admin Add Admin Screen")
    AdminSearchCourseFrame = tk.Frame(root, padx=20,pady=20)
    AdminSearchCourseFrame.pack(fill = "both", expand= True)
    ...
    def add(event=None):
        admID = IDTextBox.get().upper()
        admName = NameTextBox.get().upper()
        admSurname = SurnameTextBox.get().upper()
        admTitle= TitleTextBox.get().upper()
        admOffice = OfficeTextBox.get().upper()
        admEmail = EmailTextBox.get().upper()
        admPassword = PasswordTextBox.get().upper()
        
        try:
          sql_command = f"INSERT INTO ADMIN VALUES ({admID}, '{admName}','{admSurname}','{admTitle}','{admOffice}','{admEmail}','{admPassword}')"
          cursor.execute(sql_command)
          database.commit()
          messagebox.showwarning("Sucess", f"You Sucessfully Added {admName} To Admins!!")
        except:
          errormessage = "Error Please Fill In The Following Entries: \n"

          sql_command = f"SELECT ID FROM ADMIN"
        
          cursor.execute(sql_command)
          stuff = cursor.fetchall()
          for i in range(len(stuff)):
            print(stuff[i][0])
            if (admID == str(stuff[i][0])):
                messagebox.showwarning("Invalid", f"The ID {admID} Is Already In Use!!")
                break
        
          sql_command = f"SELECT EMAIL FROM ADMIN"
          cursor.execute(sql_command)
          stuff = cursor.fetchall()
          for i in range(len(stuff)):
            print(stuff[i][0])
            if (admEmail == str(stuff[i][0])):
                messagebox.showwarning("Invalid", f"The Email {admEmail} Is Already In USe!!")
                break
          ...
          if(admOffice == ""):
             errormessage = errormessage + "Office\n"
          if(admEmail == ""):
             errormessage = errormessage + "Email\n"
          if(admPassword == ""):
             errormessage = errormessage + "Password"
    ...
    NameLabel = Label(AdminSearchCourseFrame, text = "Name:", font='8')
    NameLabel.place(relx = 0.5, rely = 0.2, anchor = CENTER, height=50, width=700)

    NameTextBox = tk.Entry(AdminSearchCourseFrame, font='30')
    NameTextBox.place(relx = 0.5, rely = 0.2, anchor = CENTER, height=30, width=500)
       
    SurnameLabel = Label(AdminSearchCourseFrame, text = "Surname:", font='8')
    SurnameLabel.place(relx = 0.5, rely = 0.3, anchor = CENTER, height=50, width=700)

    SurnameTextBox = tk.Entry(AdminSearchCourseFrame, font='30')
    SurnameTextBox.place(relx = 0.5, rely = 0.3, anchor = CENTER, height=30, width=500)
    
    TitleLabel = Label(AdminSearchCourseFrame, text = "Title:", font='8')
    TitleLabel.place(relx = 0.5, rely = 0.4, anchor = CENTER, height=50, width=700)

    TitleTextBox = tk.Entry(AdminSearchCourseFrame, font='30')
    TitleTextBox.place(relx = 0.5, rely = 0.4, anchor = CENTER, height=30, width=500)
    
    OfficeLabel = Label(AdminSearchCourseFrame, text = "Office:", font='8')
    OfficeLabel.place(relx = 0.5, rely = 0.5, anchor = CENTER, height=50, width=700)

    OfficeTextBox = tk.Entry(AdminSearchCourseFrame, font='30')
    OfficeTextBox.place(relx = 0.5, rely = 0.5, anchor = CENTER, height=30, width=500)
   
    EmailLabel = Label(AdminSearchCourseFrame, text = "Email:", font='8')
    EmailLabel.place(relx = 0.5, rely = 0.6, anchor = CENTER, height=50, width=700)

    EmailTextBox = tk.Entry(AdminSearchCourseFrame, font='30')
    EmailTextBox.place(relx = 0.5, rely = 0.6, anchor = CENTER, height=30, width=500)
   
    PasswordLabel = Label(AdminSearchCourseFrame, text = "Password:", font='8')
    PasswordLabel.place(relx = 0.5, rely = 0.7, anchor = CENTER, height=50, width=700)
  ...
  def pick_user_type(self):
    root.title("User Type Selection")
    UserTypeFrame = tk.Frame(root, padx=20,pady=20)
    UserTypeFrame.pack(fill = "both", expand= True)
    
    def Logout(event=None):
      root.bind('<Return>', enter)
      UserTypeFrame.pack_forget()
      AdminWindow(self.ID, self.name, self.surname)

   
    PickLabel = Label(UserTypeFrame, text = "Pick A User Type", font='20')
    PickLabel.place(relx = 0.5, rely = 0.11, anchor = CENTER)

    StudentButton = tk.Button(UserTypeFrame, text = "Student", height=2, width=15, font='30', command = lambda:[UserTypeFrame.pack_forget(), self.sys_add_student()])
    StudentButton.place(relx = 0.5, rely = 0.3, anchor = CENTER)
    
    InstructorButton = tk.Button(UserTypeFrame, text = "Instructor", height=2, width=15, font='30',command = lambda:[UserTypeFrame.pack_forget(), self.sys_add_Instructor()])
    InstructorButton.place(relx = 0.5, rely = 0.45, anchor = CENTER)

    AdminButton = tk.Button(UserTypeFrame, text = "Admin", height=2, width=15, font='30',command = lambda:[UserTypeFrame.pack_forget(), self.sys_add_Admin()])
    AdminButton.place(relx = 0.5, rely = 0.6, anchor = CENTER)

    LogoutButton = tk.Button(UserTypeFrame, text = "Return", height=2, width=15, font='30', command = Logout)
    LogoutButton.place(relx = 0.5, rely = 0.9, anchor = CENTER)
...
def StudentWindow(ID, name, surname):
    ...
    SearchCourseButton = tk.Button(StudentFrame, text = "Search For a Course", height=2, width=25, font='30', command=sequence(StudentFrame.pack_forget(), u.searchcourse))
    SearchCourseButton.place(relx = 0.5, rely = 0.3, anchor = CENTER)

    AddCourseButton = tk.Button(StudentFrame, text = "Add a Course", height=2, width=25, font='30', command=sequence(StudentFrame.pack_forget(), u.addcourse))
    AddCourseButton.place(relx = 0.5, rely = 0.5, anchor = CENTER)

    DropCourseButton = tk.Button(StudentFrame, text = "Drop a Course", height=2, width=25, font='30', command=sequence(StudentFrame.pack_forget(), u.dropcourse))
    DropCourseButton.place(relx = 0.5, rely = 0.5, anchor = CENTER)

    ScheduleButton = tk.Button(StudentFrame, text = "Look at Schedule", height=2, width=25, font='30', command=sequence(StudentFrame.pack_forget(), u.show_schedule))
...
def InstructorWindow(ID, name, surname):
    ...
    SearchCourseButton = tk.Button(InstructorFrame, text = "Search For a Course", height=2, width=25, font='30', command=sequence(InstructorFrame.pack_forget(), u.searchcourse))
    SearchCourseButton.place(relx = 0.5, rely = 0.3, anchor = CENTER)

    ClassListButton = tk.Button(InstructorFrame, text = "Look at Class List", height=2, width=25, font='30', command=sequence(InstructorFrame.pack_forget(), u.show_classlist))
    ClassListButton.place(relx = 0.5, rely = 0.5, anchor = CENTER)

    ScheduleButton = tk.Button(InstructorFrame, text = "Look at Schedule", height=2, width=25, font='30', command=sequence(InstructorFrame.pack_forget(), u.show_schedule))
    ScheduleButton.place(relx = 0.5, rely = 0.7, anchor = CENTER)
...
def AdminWindow(ID, name, surname):
    ...
    def Logout(event=None):
        ...
        root.bind('<Return>', lambda:[AdminFrame.pack_forget(),enter()])
        LoginFrame.pack(fill = "both", expand= True)
    ...
    AddCourseButton = tk.Button(AdminFrame, text = "Add a Course to the System", height=1, width=30, font='30', command=sequence(AdminFrame.pack_forget(), u.sys_addCourse))
    AddCourseButton.place(relx = 0.5, rely = 0.2, anchor = CENTER)

    DropCourseButton = tk.Button(AdminFrame, text = "Drop a Course from the System", height=1, width=30, font='30', command=sequence(AdminFrame.pack_forget(), u.sys_dropCourse))
    DropCourseButton.place(relx = 0.5, rely = 0.3, anchor = CENTER)

    AddUserButton = tk.Button(AdminFrame, text = "Add a User to the System", height=1, width=30, font='30', command=sequence(AdminFrame.pack_forget(), u.pick_user_type))
    AddUserButton.place(relx = 0.5, rely = 0.4, anchor = CENTER)

    DropUserButton = tk.Button(AdminFrame, text = "Drop a User from the System", height=1, width=30, font='30', command=sequence(AdminFrame.pack_forget(), u.sys_drop_user))
    DropUserButton.place(relx = 0.5, rely = 0.5, anchor = CENTER)

    AddStudentToCourseButton = tk.Button(AdminFrame, text = "Add a Student to a Course", height=1, width=30, font='30', command=sequence(AdminFrame.pack_forget(), u.course_add_student))
    AddStudentToCourseButton.place(relx = 0.5, rely = 0.6, anchor = CENTER)

    DropStudentToCourseButton = tk.Button(AdminFrame, text = "Drop a Student From a Course", height=1, width=30, font='30', command=sequence(AdminFrame.pack_forget(), u.course_drop_student))
    DropStudentToCourseButton.place(relx = 0.5, rely = 0.7, anchor = CENTER)

    CourseRosterButton = tk.Button(AdminFrame, text = "Look at a Course Roster", height=1, width=30, font='30', command=sequence(AdminFrame.pack_forget(), u.course_roster))
